...
int add_to_existingBlock(BF_Block *block, int fd, Record record, int block_capacity, int block_num) {
    ...
    if(block_info->next_block != NULL) {
        block_num++;
        block_num = add_to_existingBlock(block_info->next_block, fd, record, block_capacity, block_num);
        CALL_OR_DIE(BF_UnpinBlock(block));
...
int HT_CreateFile(char *fileName, int buckets) {
    ...
    void *data;
    CALL_OR_DIE(BF_AllocateBlock(fd1, block1));
...
int HT_InsertEntry(HT_info *ht_info, Record record) {
    ...
    if(ht_info->hash_table[bucket] == 0) {  // Άμα δεν έχει ανατεθεί block σ αυτό το bucket αρχικοποιούμε καινούριο block και το αναθέτουμε 
        ...
        add_to_newBlock(block, ht_info->fileDesc, record);
        printf("block attached record added");
    }
        ...
        printf("bucket with attached block: %d ", ht_info->hash_table[bucket]);
        block_id = add_to_existingBlock(block, ht_info->fileDesc, record, ht_info->block_capacity, ht_info->hash_table[bucket]);
        printf("record added to block %d", block_id);
    }
...
int HT_GetAllEntries(HT_info *ht_info, void *value) {
    ...
    if(ht_info->hash_table[bucket] == 0)
        ...
        CALL_OR_DIE(BF_GetBlock(ht_info->fileDesc, ht_info->hash_table[bucket], block));
        block_read = print_records(block, ht_info->fileDesc, *id, ht_info->block_capacity, ht_info->hash_table[bucket], 0);
    }
